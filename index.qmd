...
# main data
elements = read_csv("data/gw05_elements.csv") 
stats = read_csv("data/gw05_stats.csv")
scores = read_csv("data/gw05_scoreboard.csv") 
scoreboard = read_csv("data/gw05_scores.csv") 
...
scores %>% 
  ...
  mutate(manager = if_else(manager == "Ethan", "Ethan \U2605 \U2605", manager)) |> 
...
# gameweek 0
gw0 = tibble(
  manager = c("Ben", "Christian", "Ethan", "Patrick", "Scott"),
  round = rep(0, 5),
  gw_points = rep(0, 5),
  hit = rep(0,5),
  pob = rep(0,5),
  adj_points = rep(0,5),
  total_points = rep(0,5),
  average_entry_points = rep(0,5),
  points_plus = rep(0,5)
)
...
scores %>% 
  ...
  scale_color_manual(values = c("#38003C","#EDBB00","#6CABDD","#C8102E","#00FF85"), guide = "none")+
  theme_minimal()+
  theme(
    plot.title = element_text(face = "bold", size = 14),
