...
scores = read_csv("data/gw05_scores.csv")
```
...
recent_gameweek = scores %>%
  ...
  filter(round == max(round)) %>% 
  mutate(points_plus = round(adj_points/average_points*100)) %>% 
  select(manager, adj_points, points_plus) %>% 
  arrange(desc(adj_points))
...
best_gameweek = scores %>% 
  left_join(average_scores, by = "round") %>% 
  mutate(points_plus = round(adj_points/average_points * 100)) %>% 
  select(manager, round, adj_points, points_plus) %>% 
  arrange(desc(adj_points)) %>% 
  ungroup() %>% 
  top_n(n = 10)
...
wins = scores %>% 
  group_by(round) %>% 
  mutate(won_gameweek = adj_points == max(adj_points)) %>% 
  group_by(manager) %>% 
